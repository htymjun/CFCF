  ...
  pure attributes(device) function KEEP_IGR2(rho, u, v, w, uu, p, T, sigma, Normal) result(F)
    ...
    real(8), intent(in), dimension(5) :: Normal
    real(8) F(5)
    !F(1) = 0.25d0 * (rho(1) + rho(2)) * (uu(1) + uu(2)) ! KEEP A
    F(1) = sqrt(rho(1) * rho(2)) * 0.5d0 * (uu(1) + uu(2)) ! KEEP G
    F(2) = 0.5d0 * (F(1) * (u(1) + u(2)) + (p(1) + p(2) + sigma(1) + sigma(2)) * Normal(2))
    F(3) = 0.5d0 * (F(1) * (v(1) + v(2)) + (p(1) + p(2) + sigma(1) + sigma(2)) * Normal(3))
    F(4) = 0.5d0 * (F(1) * (w(1) + w(2)) + (p(1) + p(2) + sigma(1) + sigma(2)) * Normal(4))
    !F(5) = F(1) * 0.5d0 * (T(1) + T(2)) * R_over_gamma_1 ! internal energy KEEP A
    F(5) = F(1) * sqrt(T(1) * T(2)) * R_over_gamma_1 ! internal energy KEEP G
    F(5) = F(5) + 0.5d0 * (uu(1) * (p(2) + sigma(2)) + uu(2) * (p(1) + sigma(1))) ! pressure diffusion
    F(5) = F(5) + 0.5d0 * F(1) * (u(1) * u(2) + v(1) * v(2) + w(1) * w(2)) ! kinetic energy
  ...
  pure attributes(device) function KEEP_IGR4(rho, u, v, w, uu, p, T, sigma, Normal) result(F)
    ...
    block
      real(8) pres
      pres = -one_twelfth * (p(1) + sigma(1)) + seven_twelfth * (p(2) + p(3) + sigma(2) + sigma(3)) - one_twelfth * (p(4) + sigma(4))
      F(2) = -RV3 * u1 + RV1_RV2 * u2 + RV1_RV3 * u3 - RV2 * u4 + pres * Normal(2)
      F(3) = -RV3 * v1 + RV1_RV2 * v2 + RV1_RV3 * v3 - RV2 * v4 + pres * Normal(3)
      F(4) = -RV3 * w1 + RV1_RV2 * w2 + RV1_RV3 * w3 - RV2 * w4 + pres * Normal(4)
    end block
    block
      ...
      ene  = ene + ((RV1 * (u2*u3 + v2*v3 + w2*w3)) &
                  - (RV2 * (u2*u4 + v2*v4 + w2*w4) &
                   + RV3 * (u1*u3 + v1*v3 + w1*w3)))
      ! pressure diffusion
      F(5) = ene + (two_third * (uu(2)*(p(3) + sigma(3))+uu(3)*(p(2) + sigma(2))) &
                - one_twelfth * (uu(2)*(p(4) + sigma(4))+uu(4)*(p(2) + sigma(2)) &
                                +uu(1)*(p(3) + sigma(3))+uu(3)*(p(1) + sigma(1))))
    end block
  ...
  pure attributes(device) function KEEP_IGR6(rho, u, v, w, uu, p, T, sigma, Normal) result(F)
    ...
    block
      real(8) pres, ruu, ruv, ruw
      pres = (p(1) + sigma(1) - 8.d0 * (p(2) + sigma(2)) + 37.d0 * (p(3) + p(4) + sigma(3) + sigma(4)) - 8.d0 * (p(5) + p(6) + sigma(5) + sigma(6))) * one_60
      ruu = RV6*u(1) + RV3_RV5*u(2) + RV1_RV2_RV4*u(3) + RV1_RV3_RV6*u(4) + RV2_RV5*u(5) + RV4*u(6)
      ruu = ruu + pres * Normal(2)
      ruv = RV6*v(1) + RV3_RV5*v(2) + RV1_RV2_RV4*v(3) + RV1_RV3_RV6*v(4) + RV2_RV5*v(5) + RV4*v(6)
      ruv = ruv + pres * Normal(3)
    ...
    block
      ...
      ene = ene + RV1 * (u(3)*u(4) + v(3)*v(4) + w(3)*w(4)) &
               ...
               + (RV4 * (u(3)*u(6) + v(3)*v(6) + w(3)*w(6)) &
                ...
                + RV6 * (u(1)*u(4) + v(1)*v(4) + w(1)*w(4)))
      ! pressure diffusion
      F(5) = ene + (0.75d0 * (uu(3)*(p(4) + sigma(4)) + uu(4)*(p(3) + sigma(3))) &
                  - 0.15d0 * (uu(3)*(p(5) + sigma(5)) + uu(5)*(p(3) + sigma(3)) &
                            + uu(2)*(p(4) + sigma(4)) + uu(4)*(p(2) + sigma(2))) &
                           + (uu(3)*(p(6) + sigma(6)) + uu(6)*(p(3) + sigma(3)) &
                            + uu(2)*(p(5) + sigma(5)) + uu(5)*(p(2) + sigma(2)) &
